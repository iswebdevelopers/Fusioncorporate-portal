@if(!empty($settings))
	...
	@if(!empty($sticky))
		...
		@for ($i=1; $i <= $sticky['quantity']; $i++)
				...
				^FO{{$margin_left + (($label_no - 1) * $width)}},{{$margin_top * 2.5}}^FD{{$sticky['stockroomlocator']}}^FS
				...
				^CF0,{{$font_2}},
				^FO{{$margin_left + (($label_no - 1) * $width)}},{{$margin_top * 6.5}}^FH^FD{{str_replace('~','_7e',$sticky['description'])}}^FS
				^CF0,{{$font_2}},
				^FO{{$margin_left + (($label_no - 1) * $width)}},{{$margin_top * 7}}^FH^FD{{str_replace('~','_7e',$sticky['colour'])}}^FS
